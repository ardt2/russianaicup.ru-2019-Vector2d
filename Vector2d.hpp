...
class TAngle180
{
	...
	public:
		...
		// angle приходит от -PI до PI, в данном случае можно не провер€ть.
		// ----------------------------------------------------------
...
class Point2 : public Vec2Double
{
	...
	// --------------------------------------------------------------
	public:
		// Point2() = delete; // Ћадно, снижу требовани€ дл€ удобства.
		// Ёто чтобы не было неинициализированных переменных. “ут бы нужен assert времени компил€ции. )
		...
		// ------------------------------------------------
		Point2(const Point2& p = Point2(0, 0), bool isnull = false) // » Point() 
			: Vec2Double(p.x, p.y),
			  IsNullPoint(isnull)
		{
		}
...
class Vect2 : public Point2
{
	...
	public:
		...
		//{ 
		// Ќе€вное преобразование из double! ћожно explicit поставить, но сделаем Vect2() по другому.
		//}

		// ------------------------------------------------
		//explicit Vect2(const Point2& p = ZeroPoint) : Point2(p)
		//{
		// «апрещено дл€ разделени€ точки и вектора.
		//}
		...
		// ------------------------------------------------
		// explicit здесь - следствие разделени€ точки и вектора. ¬ паре мест придетс€ (Vect2)v написать.
		...
		// ----------------------------------------------------------
		Vect2 operator-() const // –едко, но используетс€.
		{
			return Vect2(-x, -y);
		}
...
// Rect2
// ============================================================================
// ѕозици€ (Point2) - середина нижней стороны.
